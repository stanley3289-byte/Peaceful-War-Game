...
    return {
        // 初始狀態：HP 4, 盾 0, 炮 無
        player1: { hp: 4, shields: 0, hasTurret: false, rpsChoice: null, name: 'Player1' },
        player2: { hp: 4, shields: 0, hasTurret: false, rpsChoice: null, name: 'Player2' },
        // 遊戲一開始就進入 RPS 階段
...
                
            } else {
                // 如果沒有炮台，則起盾或起炮 (簡單 AI 策略)
                if (actorData.shields >= 3 && !actorData.hasTurret) { 
...
        if (actor.shields >= 3 && !actor.hasTurret) {
             actor.hasTurret = true;
             message = `${actor.name} 建造了砲台！`;
        } else {
             message = `${actor.name} 建造砲台失敗 (盾牌不足或已有砲台)。`;
...
                // Log 格式包含 "摧毀了" 用於觸發客戶端動畫和音效
                message = `${actor.name} 摧毀了對手的砲塔！`; 
